...
namespace VFEMedieval
{
    ...
    public class CompEditHeraldic : ThingComp
    {
        public override IEnumerable<Gizmo> CompGetGizmosExtra()
        {
            ...
            if (parent.Faction == Faction.OfPlayerSilentFail && (parent is Pawn == false || WorldComponent_HeraldryPawns.Instance.heraldry_pawns.Contains(parent)))
            {
        ...
        private List<Color> AllColors
        {
            get
            {
                if (allColors == null)
                {
                    ...
                    if (ModsConfig.IdeologyActive && Pawn?.story != null && !Pawn.DevelopmentalStage.Baby() && Pawn.story.favoriteColor!=null && !allColors.Any((Color c) => Pawn.story.favoriteColor.color.IndistinguishableFrom(c)))
                    {
                        allColors.Add(Pawn.story.favoriteColor.color);
                    }
        ...
        private void DrawHeraldicSetup(Rect rect)
        {
            ...
            foreach ((string type, var heraldicPart) in activeHeraldry.Items)
            {
                if (heraldicPart is TaggedColor tColor)
                {
                    ...
                    if (Pawn?.story is Pawn_StoryTracker story && story.favoriteColor != null)
                    {
                        colorRect = new Rect(num2, curY, 160f, 24f);
